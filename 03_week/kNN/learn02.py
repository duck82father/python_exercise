...
import os
import pandas as pd
# 데이터의 시각화와 관련된 주요 작업을 seaborn을 통해 수행하면서 동시에 그래프의 디테일한 조정이나 추가적인 수정을 위해 matplotlib의 기능을 활용
...
# sns.lmplot(): 데이터 포인트를 2차원 평면에 그리기 위해 사용.
# 이 함수는 matplotlib의 scatter 플롯 위에 회귀선을 추가하는 기능도 있지만, 여기서는 fit_reg=False로 설정하여 회귀선을 그리지 않았습니다.
...
plt.title("STL and 2P in 2d plane") # 그래프의 제목을 설정
